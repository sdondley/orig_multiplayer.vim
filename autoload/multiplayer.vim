...
function! multiplayer#Connect(profile_file, player_profile)
	...
	let s:sleep_job = jobstart(['/bin/sh', '-c', 'sleep infinity > /tmp/vim_multi_player_pipe_' . my_pid])
	sleep 100m " make sure sleep keeps the cat alive
	call jobstart('cat /tmp/vim_multi_player_pipe_' . my_pid, {"out_cb": function("s:MyHandlerOut")})
